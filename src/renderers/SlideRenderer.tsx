    ...
    {items.map((item, i) => (
      <li key={i} style={{ margin: '0.38em 0', fontSize: '0.82em', lineHeight: 1.5, counterIncrement: 'sl', paddingLeft: '2.3em', position: 'relative' }}>
        <span style={{
          ...
          width: '1.7em', height: '1.7em', background: 'var(--sl-accent)', color: '#fff',
...
const thm = () => ({
  bg: 'var(--sl-bg, white)',
  surf: 'var(--sl-surf, #f5f5f5)',
  accent: 'var(--sl-accent, #4361ee)',
  a2: 'var(--sl-a2, #f72585)',
  tx: 'var(--sl-tx, #333)',
  tx2: 'var(--sl-tx2, #999)',
  hd: 'var(--sl-hd, #111)',
  bd: 'var(--sl-bd, #e0e0e0)',
  grd: 'var(--sl-grd, #f0f0f0)',
  ok: 'var(--sl-ok, #22c55e)',
  err: 'var(--sl-err, #ef4444)',
  wrn: 'var(--sl-wrn, #f59e0b)',
})

const s: Record<string, React.CSSProperties> = {
  slide: { padding: '52px 80px 28px 80px', display: 'flex', flexDirection: 'column', minHeight: 400, overflow: 'hidden' },
  h2: { fontFamily: 'Georgia, serif', fontSize: '2.1em', fontWeight: 700, letterSpacing: '-0.025em', lineHeight: 1.18, margin: '0 0 0.45em 0' },
  ha: { display: 'inline-block', paddingBottom: '0.3em', marginBottom: '0.5em', borderBottom: '3px solid var(--sl-accent)' },
  ...
  kp: { background: 'var(--sl-surf)', border: '1px solid var(--sl-bd)', borderRadius: 14, padding: '1.4em 0.8em', textAlign: 'center' },
  ...
  cp: { background: 'var(--sl-surf)', border: '1px solid var(--sl-bd)', borderRadius: 14, padding: '1.1em 1.2em', textAlign: 'left' },
  tlItem: { position: 'relative', paddingLeft: '1.8em', marginBottom: '0.5em' },
  tlLine: { position: 'absolute', left: '0.5em', top: '0.3em', width: 2, bottom: '-0.3em', background: 'linear-gradient(180deg, var(--sl-accent), var(--sl-a2))', borderRadius: 1 },
  tlDot: { position: 'absolute', left: '0.5em', top: '0.3em', width: 10, height: 10, borderRadius: '50%', background: 'var(--sl-accent)', transform: 'translateX(-4px)' },
  ...
  fcNode: { background: 'var(--sl-surf)', border: '2px solid var(--sl-accent)', borderRadius: 14, padding: '0.6em 1em', textAlign: 'center', fontWeight: 600, fontSize: '0.82em', minWidth: 100 },
  agItem: { display: 'flex', alignItems: 'center', gap: '1em', padding: '0.5em 0', borderBottom: '1px solid var(--sl-bd)' },
  ...
  tmCard: { background: 'var(--sl-surf)', border: '1px solid var(--sl-bd)', borderRadius: 14, padding: '1em', textAlign: 'center' },
  lgItem: { background: 'var(--sl-surf)', border: '1px solid var(--sl-bd)', borderRadius: 14, padding: '0.8em', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  pgBar: { margin: '0.3em 0' },
  pgTrack: { height: 8, background: 'var(--sl-grd)', borderRadius: 4, overflow: 'hidden' },
  pgFill: { height: '100%', borderRadius: 4, background: 'linear-gradient(90deg, var(--sl-accent), var(--sl-a2))' },
}
...
function TitleSlide({ slide }: { slide: Slide }) {
  return (
    <div style={{ ...s.slide, justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: '60px 100px' }}>
      {slide.title && <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '3.6em', fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.08, margin: '0 0 0.1em 0', color: 'var(--sl-hd)' }}>{slide.title}</h1>}
      <div style={{ width: 72, height: 5, background: 'linear-gradient(90deg, var(--sl-accent), var(--sl-a2))', borderRadius: 3, margin: '0 auto 1.4em' }} />
      {slide.subtitle && <p style={{ fontSize: '1.15em', color: 'var(--sl-tx2)', maxWidth: '30em', lineHeight: 1.55 }}>{slide.subtitle}</p>}
...
function SectionSlide({ slide }: { slide: Slide }) {
  return (
    <div style={{ ...s.slide, justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
      <p style={{ fontSize: '0.36em', fontWeight: 700, color: 'var(--sl-accent)', letterSpacing: '0.22em', textTransform: 'uppercase', marginBottom: '0.9em' }}>{slide.title}</p>
      {slide.subtitle && <p style={{ fontSize: '0.95em', color: 'var(--sl-tx2)' }}>{slide.subtitle}</p>}
    </div>
  )
}
...
function QuoteSlide({ slide }: { slide: Slide }) {
  ...
  return (
    <div style={{ ...s.slide, justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
      <blockquote style={{ fontFamily: 'Georgia, serif', fontSize: '2em', fontStyle: 'italic', lineHeight: 1.4, maxWidth: '70%', border: 'none', padding: 0, color: 'var(--sl-hd)' }}>
        <span style={{ fontSize: '2.6em', color: 'var(--sl-accent)', display: 'block', marginBottom: '0.05em', lineHeight: 1 }}>&ldquo;</span>
        {q}
      </blockquote>
      {slide.subtitle && <p style={{ fontSize: '0.78em', color: 'var(--sl-tx2)', fontWeight: 600, marginTop: '1.1em' }}>&mdash; {slide.subtitle}</p>}
    </div>
  )
}
...
function StandardSlide({ slide }: { slide: Slide }) {
  return (
    <div style={s.slide}>
      {slide.title && <h2 style={{ ...s.h2, color: 'var(--sl-hd)' }}><span style={s.ha}>{slide.title}</span></h2>}
      {slide.subtitle && <p style={{ ...s.sub, color: 'var(--sl-tx2)' }}>{slide.subtitle}</p>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.85em', marginBottom: '0.5em' }}>
            {Array.from({ length: n }).map((_, i) => (
              <div key={i} style={s.kp}>
                {i < kpiVals.length && <div style={{ ...s.kv, color: 'var(--sl-accent)' }}>{(kpiVals[i] as any).content}</div>}
                {i < kpiLbls.length && <div style={{ ...s.kl, color: 'var(--sl-tx2)' }}>{(kpiLbls[i] as any).content}</div>}
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div style={s.slide}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2em', flex: 1, overflow: 'auto' }}>
        <div style={s.cp}>
          {lt && <h3 style={{ fontSize: '1.05em', textAlign: 'center', margin: '0 0 0.7em', paddingBottom: '0.45em', borderBottom: '2px solid var(--sl-accent)' }}>{lt}</h3>}
          {lb.map((b, i) => <BlockRenderer key={i} block={b} />)}
        </div>
        <div style={s.cp}>
          {rt && <h3 style={{ fontSize: '1.05em', textAlign: 'center', margin: '0 0 0.7em', paddingBottom: '0.45em', borderBottom: '2px solid var(--sl-accent)' }}>{rt}</h3>}
    ...
    <div style={{ ...s.slide, justifyContent: 'center', alignItems: 'center' }}>
      ...
      {ib ? (
        <>
          ...
          {ib.source.caption && <p style={{ fontSize: '0.62em', color: 'var(--sl-tx2)', marginTop: '0.5em', fontWeight: 500 }}>{ib.source.caption}</p>}
        </>
      ) : <p style={{ color: 'var(--sl-tx2)' }}>[No image]</p>}
      <FooterSlide title={slide.title} />
    </div>
  )
}
    ...
    <div style={s.slide}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.85em' }}>
          {Array.from({ length: n }).map((_, i) => (
            <div key={i} style={s.kp}>
              {i < vals.length && <div style={{ ...s.kv, color: 'var(--sl-accent)' }}>{(vals[i] as any).content}</div>}
              {i < lbls.length && <div style={{ ...s.kl, color: 'var(--sl-tx2)' }}>{(lbls[i] as any).content}</div>}
            </div>
          ))}
        </div>
        {rest.map((b, i) => <BlockRenderer key={i} block={b} />)}
...
function BigNumberSlide({ slide }: { slide: Slide }) {
  ...
  return (
    <div style={{ ...s.slide, justifyContent: 'center', alignItems: 'center', textAlign: 'center', padding: '50px 100px' }}>
      {val && <div style={{ fontFamily: 'Georgia, serif', fontSize: '5em', fontWeight: 900, color: 'var(--sl-accent)', lineHeight: 1, letterSpacing: '-0.04em', marginBottom: '0.1em' }}>{(val as any).content}</div>}
      {lbl && <div style={{ fontSize: '0.85em', color: 'var(--sl-tx2)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.12em' }}>{(lbl as any).content}</div>}
      {slide.subtitle && <div style={{ fontSize: '0.75em', color: 'var(--sl-tx2)', marginTop: '0.8em', maxWidth: '24em', lineHeight: 1.5 }}>{slide.subtitle}</div>}
      <FooterSlide title={slide.title} />
    </div>
  )
}
    ...
    <div style={s.slide}>
      ...
      <div style={{ flex: 1, overflow: 'auto', paddingLeft: '2.5em' }}>
        <div style={{ position: 'relative' }}>
          <div style={{ position: 'absolute', left: '0.7em', top: '0.4em', bottom: '0.4em', width: 2, background: 'linear-gradient(180deg, var(--sl-accent), var(--sl-a2))', borderRadius: 1 }} />
          {texts.map((tb: any, i) => {
            ...
            return (
              <div key={i} style={{ position: 'relative', marginBottom: '0.7em', paddingLeft: '1.2em' }}>
                <div style={{ position: 'absolute', left: '-2.5em', top: '0.45em', width: 12, height: 12, borderRadius: '50%', background: 'var(--sl-accent)', border: '2px solid var(--sl-bg)', boxShadow: '0 0 0 2px var(--sl-accent)' }} />
                <div style={{ fontSize: '0.6em', color: 'var(--sl-accent)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{date}</div>
                {title && <div style={{ fontWeight: 700, fontSize: '0.85em', margin: '0.08em 0', color: 'var(--sl-hd)' }}>{title}</div>}
                <div style={{ fontSize: '0.72em', color: 'var(--sl-tx2)', lineHeight: 1.45 }}>{desc}</div>
              </div>
            )
          })}
        </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.8em', justifyContent: 'center', flex: 1, overflow: 'auto', alignContent: 'flex-start' }}>
        {nodes.map((n: any, i) => (
          <div key={i} style={{ ...s.fcNode, position: 'relative' }}>
            {n.content}
            {i < nodes.length - 1 && <div style={{ color: 'var(--sl-accent)', fontSize: '1.1em', marginTop: '0.4em', fontWeight: 700 }}>↓</div>}
          </div>
        ))}
      </div>
      <FooterSlide title={slide.title} />
          ...
          if (b.type === 'text') {
            ...
            return (
              <div key={i} style={s.agItem}>
                <div style={{ ...s.agNum, color: 'var(--sl-accent)' }}>{String(i + 1).padStart(2, '0')}</div>
                <div>
                  <div style={{ ...s.agTitle, color: 'var(--sl-hd)' }}>{title}</div>
                  {desc && <div style={{ ...s.agDesc, color: 'var(--sl-tx2)' }}>{desc}</div>}
                </div>
              </div>
            )
          }
          ...
          return (
            <div key={i} style={s.tmCard}>
              {img ? <img src={img.source.url} alt={img.source.alt || name} style={{ width: 72, height: 72, borderRadius: '50%', objectFit: 'cover', marginBottom: '0.6em', boxShadow: '0 4px 16px rgba(0,0,0,0.1)' }} />
                : <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'var(--sl-grd)', margin: '0 auto 0.6em', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5em', color: 'var(--sl-accent)', fontWeight: 700 }}>{name.charAt(0).toUpperCase()}</div>}
              <div style={{ fontWeight: 700, fontSize: '0.85em', color: 'var(--sl-hd)' }}>{name}</div>
              {role && <div style={{ fontSize: '0.7em', color: 'var(--sl-accent)', fontWeight: 600, marginTop: '0.2em', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{role}</div>}
            </div>
          )
        })}
      </div>
          ...
          return (
            <div key={i} style={s.pgBar}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75em', fontWeight: 600, marginBottom: '0.2em' }}>
                <span>{label}</span>
                <span style={{ color: 'var(--sl-accent)' }}>{val}</span>
              </div>
              <div style={s.pgTrack}>
...
function ContactSlide({ slide }: { slide: Slide }) {
  ...
  return (
    <div style={{ ...s.slide, justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
      {slide.title && <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '3em', fontWeight: 800, color: 'var(--sl-hd)', marginBottom: '0.3em' }}>{slide.title}</h1>}
      <div style={{ width: 60, height: 4, background: 'linear-gradient(90deg, var(--sl-accent), var(--sl-a2))', borderRadius: 2, margin: '0.8em auto' }} />
      {slide.subtitle && <p style={{ fontSize: '1.1em', color: 'var(--sl-tx2)', marginBottom: '0.8em' }}>{slide.subtitle}</p>}
      {texts.map((tb: any, i) => <p key={i} style={{ fontSize: '0.85em', color: 'var(--sl-tx2)', margin: '0.3em 0', lineHeight: 1.6 }}>{tb.content}</p>)}
      <FooterSlide title={slide.title} />
    </div>
  )
}
